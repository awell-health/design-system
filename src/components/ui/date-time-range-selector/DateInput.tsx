...
const DateInput: React.FC<DateInputProps> = ({ value, onChange, disabled = false, className }) => {
  ...
  const handleInputChange =
    (field: keyof DateParts) => (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (isValid) {
        const updatedDate = new Date(newDate.year, newDate.month - 1, newDate.day);
        // Preserve time from original value
        if (value) {
  ...
  return (
    <div
      className={cn(
        'flex border border-slate-300 rounded-lg items-center text-sm px-1 bg-white',
    ...
    >
      <input
        ...
        className={cn(
          'p-0 outline-none w-6 border-none text-center bg-transparent',
      ...
      <input
        ...
        className={cn(
          'p-0 outline-none w-7 border-none text-center bg-transparent',
          'focus:ring-0 focus:outline-none'
        )}
        placeholder='D'
        disabled={disabled}
      ...
      <input
        ...
        className={cn(
          'p-0 outline-none w-12 border-none text-center bg-transparent',
          'focus:ring-0 focus:outline-none'
        )}
        placeholder='YYYY'
        disabled={disabled}
